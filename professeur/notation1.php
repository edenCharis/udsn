...
if( $_SESSION['id'] == session_id() and  $_SESSION['role']=="enseignant"){
  ...
  if(isset($_GET["classe"]) and isset($_GET["annee"])){
    ...
    if(isset($_GET['sup'])){
        ...
        $sql="delete from ligne1 where id=$id";
        ...
        if($connexion->query($sql)){
            ...
            logUserAction($connexion,$_SESSION['id_user'],"supression d'une note ",date("Y-m-d H:i:s"),$userIP,"valeur suprimée :$id");
             ...
             $anonyme=  $_GET['code'];
             $semestre = getSemestreByAnonymat($anonyme,$connexion);
             $classe=getClasseByAnonymat($anonyme,$connexion);
             $ecue=$_GET['ecue'];
             $annee=$_GET['annee'];
             $etudiant =getIdInscriptionFromAnonymat($anonyme,$annee,$connexion);
             ...
             if($id_notation !== null){
                ...
                if($type == "Session Ordinaire"){
                    $sql1="UPDATE notation set moyEx=null where id=$id_notation";

                }else{
                    $sql1="UPDATE notation set session_rappel=null where id=$id_notation";
                }

                    if($connexion->query($sql1)){
                        header("location: notation1?sucess=Suppression effectuée avec success");
                    }else{
                ...
                header("location: notation1?erreur= Erreur Survenue lors de l'opération !" );
             }
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-header">
								<h4 class="card-title">Liste des notes d'examens</h4>
								...
								<div class="table-responsive">
									<table id="example3" class="display" style="min-width: 845px">
										...
										<tbody>
                                          ...
                                          <?php 
                                                 $sql ="select * from ligne1 where anonymat in ( select a.numero from anonymat a join  repartition_enseignant r on a.classe=r.classe where r.code='".$_SESSION["code_enseignant"]."') and annee in  (select annee from repartition_enseignant where code='".$_SESSION["code_enseignant"]."')  and user_id=".$_SESSION["id_user"];
                                          ...
                                          ?>
											<tr heigth="15%">
                                                ...
                                                <td><?php echo getClasseByAnonymat($ue['anonymat'],$connexion);?></td>
												
												<td><?php echo str_replace("+","'",$ue['ecue']);?></td>
												<td><?php echo $ue['type_examen'];?></td>
                                                <td><?php echo $ue['note'];?></td>
                                                <td><?php echo getSemestreByAnonymat($ue['anonymat'],$connexion);?></td>
                                                <td><?php echo $ue['annee'];?></td>			
                                                
												<td>			
                                                   <a href="notation1?sup=<?php echo $ue['id'];?>&examen=<?php echo $ue['type_examen']?>&ecue=<?php echo $ue['code_ecue']?>&code=<?php echo $ue['anonymat'];?>&annee=<?php echo $ue['annee']?>" class="btn btn-sm btn-danger"><i class="la la-trash-o"></i>Supprimer</a>
                                                </td>												
											</tr>
...
<div class="modal fade bd-example-modal-lg" id="typeAgentModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" id="exampleModalLabel">FORMULAIRE D'ENREGISTREMENT D'UNE NOTE D'EXAMEN </h3>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                ...
                <div class="form-group">
                        <label for="codeanonyme">Code Anonyme </label>
                        <input type="text"  class="form-control" id="codeanonyme" name="codeanonyme" required>
                </div>
