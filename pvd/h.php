...
if(isset($_GET['semestre']) and isset($_GET['parcours']) and isset($_GET['annee']) and isset($_GET['examen']))
{
                    ...
                    <div id="list-view" class="tab-pane fade active show col-lg-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <?php 
      $sql = "select ue.libelle as libelle from ue join specialite on ue.Specialite=specialite.libelle join parcours on specialite.parcours=parcours.libelle where ue.etab='".$_SESSION["etablissement"]."' and ue.libelle in (select ue from ecue ) and semestre='$semestre' and parcours.libelle='$parcours'";
    ?>
...
<table id="example3" class="display" >
    ...
    ?>
        ...
        <tr>
         ...
         ?>
            <th class="small-column">  <?php   if($a < 10  and $a !== "##") { ?><input type="number" maxlength="4" value="<?php echo ($a !== "##") ? $a : "#" ;?>"><?php }else{  echo ($a !== "##") ? $a : "#" ;}?></th>
            <th class="small-column text-danger"> <?php   if($b < 10  and $a !== "##" ) { ?><input type="number" maxlength="4" value="<?php echo ($b !== "##") ? $b: "#";?>"><?php }else{  echo ($b !== "##") ? $b: "#" ; } ?> </th>
           ...
           <th class="small-column">   
            <form action="traitement6" method="post" >
            <select name="decision" class="form-control-sm" recquired>

              <option selected=""><?php  if(verifierEliminatoire($eliminatoires) == false){echo ($result_ue->num_rows - 1 >=1) ?  statutSoutenance(round($points / ($result_ue->num_rows - 1),2)):"";}else{ echo "Note Eliminatoire";}?></option>
             <?php  if(verifierEliminatoire($eliminatoires) == false){?> <option ><?php echo  (($result_ue->num_rows - 1 >=1) ?  (statutSoutenance(round($points / ($result_ue->num_rows - 1),2)) == "Ajourné" ? "Admis" : "Ajourné"):"");?></option><?php }else{?>
                <option >Admis</option>
                <option >Ajourné</option>
                <?php }?>
            
           </select> 
            <input type="hidden" name="etudiant" value="<?php echo $etudiant->id;?>">  
            ...
            <input type="hidden" name="moy" value="<?php echo  ($result_ue->num_rows - 1 >=1) ?  round($points / ($result_ue->num_rows - 1),2):"";?>">  
          
              <button type="submit" name="changer"  class="btn btn-sm btn-success"> <i class="la la-edit"></i></button></th>
           </form>
