...
<table id="example3" class="display" >
    ...
    <?php 
          ...
          $sql1="select inscription.id as id from inscription join classe on inscription.classe=classe.libelle join specialite on classe.specialite=specialite.libelle join parcours on specialite.parcours=parcours.libelle where classe.niveau='$niveau' and parcours.libelle='$parcours' and classe='$classe'";
    ...
    <form action="traitement6" method="post">
        <select name="decision" class="form-control-sm" required>
            <?php  
            if (verifierEliminatoire($eliminatoires)) {
                $statut_actuel = "Note Eliminatoire";
            } else {
                $statut_actuel = statutSoutenance($tt);
            }
            ?>
            <option selected=""><?php echo $statut_actuel; ?></option>

            <?php  
            // Afficher les autres statuts possibles sans répéter le statut actuel
            $possibilites = ["Ajourné", "Admis", "Note Eliminatoire"];
            foreach ($possibilites as $possibilite) {
                if ($possibilite !== $statut_actuel) {
                    echo "<option>$possibilite</option>";
                }
            }
            ?>
        </select>

        <input type="hidden" name="etudiant" value="<?php echo htmlspecialchars($etudiant->id);?>">
        <input type="hidden" name="parcours" value="<?php echo htmlspecialchars($parcours);?>">
        ...
        <input type="hidden" name="examen" value="<?php echo htmlspecialchars($examen);?>">
        <input type="hidden" name="moy" value="<?php echo $tt;?>">

        <button type="submit" name="changer" class="btn btn-sm btn-success">
